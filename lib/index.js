...
const { attrsToQuery, cacheFile } = require('./codegen/utils')
...
module.exports = function (source) {
  ...
  if (descriptor.script) {
    ...
    const componentOptions = new Function('var exports = {};function require(src) { return src; }' + code + '\nreturn exports.default')()
    if (componentOptions.components) {
      const template = descriptor.template.content || ''
      Object.keys(componentOptions.components).forEach(key => {
        const cpath = componentOptions.components[key]
        componentOptions.components[key] = {
          path: path.extname(cpath) ? cpath : cpath + '.vue',
          inTemplate: !!template.match(new RegExp(`\<${key}.*\>`))
        }
      })
    }
    // 缓存文件
    cacheFile(src, componentOptions)
    // console.log()
  ...
  // 缓存组件样式长度
  cacheFile(resourcePath + 'styles', !!stylesCode)
  let code = `
${templateImport}
${scriptImport}
${stylesCode}
